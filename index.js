...
async function initDb() {
  ...
  try {
    ...
    console.log('[init] Verificando escala de folga (ago-dez/2026)...');
    for (const mes of Object.keys(FOLGA_DATA_MESES)) {
      const dadosDoMes = FOLGA_DATA_MESES[mes];
      for (const matricula of Object.keys(dadosDoMes)) {
        const dias = dadosDoMes[matricula];
        for (let i = 0; i < dias.length; i++) {
          if (dias[i] === 'F') {
            const data = isoDateSeed(mes, i);
            await client.query(
